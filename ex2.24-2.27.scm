...
; this works properly due to the use of (list (deep-reverse-r (car tree)))
; 
...
(define (deep-reverse-r2 tree)
  (cond	((not (pair? tree)) tree)
	((null? (cdr tree)) (list (deep-reverse-r2 (car tree))))
	(else (append
	       (deep-reverse-r2 (cdr tree))
	       (list (deep-reverse-r2 (car tree)))))))

(deep-reverse-r2 (list 1 2 3 3 5))

(deep-reverse-r2 x)

(deep-reverse-r2 (list 1 x 2 x 3))
